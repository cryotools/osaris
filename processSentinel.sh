...
for swath in ${swaths_to_process[@]}; do
    cd $work_PATH
    rm -rf F$swath/raw; mkdir -pv F$swath; cd F$swath; mkdir -pv raw; mkdir -pv topo; cd topo; ln -s $topo_PATH/dem.grd .;
done
	...
	do
	    ...
	    if [ "$dataline_count" -eq 1 ]; then
	    	...
	    	if [ "$parallel_processing" -eq 1 ]; then
	    	    ...
	    	    sbatch $GSP_directory/PP-start-S1A $previous_scene $previous_orbit $current_scene $current_orbit $swath $work_PATH $GSP_directory $gmtsar_config_file $dataline_count 
