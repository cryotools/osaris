...
if ($#argv < 3) then
    echo ""
    echo "Usage: p2p_ERS.csh master_stem slave_stem configuration_file"
    echo ""
    echo "Example: p2p_ERS.csh ERS_127_2943_61103 ERS2_127_2943_65111 config.ers.txt"
    echo ""
    echo "         Place the raw data in a directory called raw and a dem.grd file in "
    echo "         a parallel directory called topo.  The raw data should have 4 files "
    echo "         - two with a suffix of .dat and two with a suffix of .ldr. "
    echo "         Execute this command at the directory location above raw and topo. "
    echo "         The file dem.grd is a dem that completely covers the SAR frame - larger is OK."
...
# 
  ...
  set near_range = `grep near_range $3            |   awk '$1 !~ /#/{if ($2 = "=" && $1 == "near_range" ) print $3}'| awk 'END {print $0}'`
...
#
    echo ""
    echo " PREPROCESS ERS DATA  -- START"
    cd raw
    echo " ERS_pre_process $master $near_range $earth_radius $npatch $fd"
    ERS_pre_process $master $near_range $earth_radius $npatch $fd
    set NEAR = `grep near_range $master.PRM | awk '{print $3}'`
    set RAD = `grep earth_radius $master.PRM | awk '{print $3}'`
    ERS_pre_process $slave $NEAR $RAD $npatch $fd
#   
...
# 
      if ($shift_topo == 1) then 
        ...
        offset_topo amp-$master.grd topo_ra.grd 0 0 11 topo_shift.grd 
        cd ..
