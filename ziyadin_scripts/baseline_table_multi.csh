...
#
if ($SSC == 1 || $SSC == 2) then
 ERS_baseline $1 $2 > $1:r.temp
 @ YR = $T0  / 1000 - 1992
 @ YDAY = $YR * 365 + $DAY
else if ($SSC == 4) then
 ENVI_baseline $1 $2 > $1:r.temp
 @ YR = $T0  / 1000 - 1992
 @ YDAY = $YR * 365 + $DAY
else if ($SSC == 5) then
 ALOS_baseline $1 $2 > $1:r.temp
 @ YR1 = $T0 / 1000
 if ($YR1 < 2013) then
  @ YR = $T0  / 1000 - 2006
 else
...
#
#  get the needed parameters from $1:r.temp
#
 set BPL = `grep B_parallel $1:r.temp | awk '{print $3}'`
 set BPR = `grep B_perpendicular $1:r.temp | awk '{print $3}'`
 set XS  = `grep xshift $1:r.temp | awk '{print $3}'`
 set YS  = `grep yshift $1:r.temp | awk '{print $3}'`
